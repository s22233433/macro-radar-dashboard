...
assert.equal(dataset.reportCount, 5);
assert.equal(dataset.latest.date, "2026-06-20");
assert.equal(dataset.previous.date, "2026-06-17");
assert.equal(dataset.timeSeries.DXY.length, 5);
assert.deepEqual(
  dataset.timeSeries.DXY.map((point) => point.date),
  ["2026-06-15", "2026-06-16", "2026-06-16", "2026-06-17", "2026-06-20"]
);
assert.equal(dataset.reports.findIndex((report) => report.filename === "macro-radar-decision-2026-06-16.md"), 2);
assert.equal(dataset.reports.findIndex((report) => report.filename === "macro-radar-daily-2026-06-16.md"), 3);
assert.ok(
...
assert.ok(script.includes('"reportCount": 5'));
